...
- 4 clés `REACT_TO_REGISTRY` (`bin/generate-registry.js`) : `Alert→alert`, `Skeleton→skeleton`, `EmptyState→empty-state`, `Spinner→spinner`. **6 entrées** registre `react: pending → ported` (les 4 composants + `zone-banner`/`upgrade-prompt`, passées manuellement faute de dossier propre — 2 clés fictives `Alert__zone-banner`/`Alert__upgrade-prompt` ajoutées à `REACT_TO_REGISTRY` uniquement pour satisfaire la vérification réciproque du scanner anti-fantôme, jamais scannées car leur dossier n'existe pas). `node bin/generate-registry.js --check` : rc=0, 0 classe fantôme, 0 dérive.
