...
- **Repli de rail en pied (#920)** — démo `navigation.html` : `.rail-toggle` dans `.rail-footer`. **Aucune classe CSS nouvelle** ; la classe n'était pas scopée au header, elle s'y comportait déjà correctement. Le blocage était entièrement côté React (cf. `togglePosition`).
...
- **`.tabs` et `.stepper` masquaient inconditionnellement leur scrollbar, comme `.segmented` avant #866 (#900)** — les trois composants partageaient le même triplet hérité de #530 : `overflow-x: auto` + `scrollbar-width: none` + `::-webkit-scrollbar { display: none }`. Le scroll fonctionnait donc réellement, mais **aucune affordance ne signalait qu'il y avait du contenu hors champ**, et une souris standard sans molette horizontale ne pouvait pas atteindre les onglets/étapes coupés. Deux sprints consécutifs que le constat était reporté côté consommateur (KeepThread). Correctif calqué à l'identique sur celui de `.segmented` : `scrollbar-width: thin` + `scrollbar-color` (Firefox) et styling manuel de `::-webkit-scrollbar` (Chromium/Safari, 4px, track transparent, thumb `var(--border-color)`). La scrollbar n'apparaît que si le contenu déborde réellement — **aucun changement visuel** pour les instances qui tiennent déjà dans leur conteneur.
  - **Portée mesurée, pas supposée** (sonde Playwright sur la démo « conteneur étroit ») : en **Chromium 147** headless, `scrollbar-width` ne réserve **aucune** place (`offsetHeight - clientHeight` = bordures seules) — les scrollbars y sont en mode *overlay* et se matérialisent au scroll/survol au lieu d'être permanentes ; **Firefox** réserve bien une gouttière fine. Ce que le correctif rétablit dans tous les cas, c'est le **mécanisme** : `scrollbar-width: none` (#530) supprimait la scrollbar inconditionnellement, partout, ne laissant strictement rien à saisir à la souris — le navigateur décide désormais de son rendu. Le même constat vaut pour `.segmented` (#866, déjà mergé) : il n'est pas propre à ce correctif.
...
### Fixed
- **`initBottomSheet` — `aria-modal="true"` mensonger** (`shared/components.js`, #825) : à l'ouverture, le composant posait `role="dialog"` + `aria-modal="true"` sur le panneau puis appelait `panel.focus && panel.focus()` — un no-op silencieux, `.bottom-sheet` (`pages/overlays.html`) n'ayant aucun `tabindex` dans le markup ni posé par le JS, un `<div>` sans `tabindex` n'étant pas focusable au script. La garde `panel.focus &&` teste l'existence de la méthode (toujours vraie sur un élément DOM), elle ne protégeait de rien. Conséquence réelle : le focus restait sur le déclencheur derrière l'overlay, Tab continuait de parcourir l'arrière-plan que `aria-modal="true"` déclarait pourtant inerte, et rien ne restaurait le focus à la fermeture.

  Corrigé : `tabindex="-1"` posé **par le JS**, dans la boucle de normalisation d'`initBottomSheet` — pas dans le markup de la vitrine. Trois panneaux de démo existent déjà dans `pages/overlays.html` ; le DS distribue ce markup à des consumers, un attribut que seul le JS pose évite qu'ils doivent penser à le recopier dans leur propre instanciation. Le focus est déplacé sur le panneau à l'ouverture, piégé (Tab/Shift+Tab) tant qu'il reste ouvert (liste de sélecteurs focusables classique du pattern WAI APG dialog), et restauré sur le déclencheur à la fermeture — bouton fermer, clic overlay, `Échap`, glissement (swipe) : les 4 chemins convergent tous vers `closeSheet()`, qui restaure dans tous les cas.

  Réutilisation du mécanisme d'`initModals` : `attachFocusRestore` capturait/restaurait déjà `document.activeElement` autour de `showModal()`/l'événement `close` d'une `<dialog>` — mais `.bottom-sheet` est un `<div>`, sans `showModal`/`close` natifs à hooker. Son cœur (capture/restore) a été extrait dans une fonction générique `createFocusRestore()`, partagée telle quelle par `attachFocusRestore` (dialogs) et `initBottomSheet` (`openSheet`/`closeSheet`, fonctions maison), plutôt que d'écrire un second mécanisme de restauration à côté. Comportement d'`attachFocusRestore` inchangé — les 8 tests existants de `modals.test.js` passent toujours à l'identique.

  Tests : `tests/vanilla/bottom-sheet.test.js` +8 (tabindex posé à l'init, focus à l'ouverture, restore par bouton/overlay/`Échap`/swipe, piège Tab et Shift+Tab). Preuve par mutation : les 8 tests sont rouges sans le correctif (composant construit itérativement contre eux), verts avec.

- **Tabs — `Home`/`End` absents** (`shared/components.js`, bloc inline `initComponents`, #826) : le listener clavier des onglets gérait `Enter`/`Espace`/`ArrowRight`/`ArrowLeft`, mais pas `Home`/`End` — contrairement à `initSegmentedControls`, qui les gère depuis #613 (même famille d'interaction, deux niveaux de service). `Home` active désormais le premier onglet, `End` le dernier, en réutilisant l'activation existante (`tabs[0].click()` / `tabs[tabs.length-1].click()`) sans dupliquer la logique de sélection. Tests : `tests/vanilla/tabs.test.js` +3 (Home, End, Home idempotent sur le 1er onglet déjà actif). Preuve par mutation : retrait du bloc `Home`/`End` → 3 tests rouges, 7 tests tabs préexistants toujours verts ; correctif restauré → 26/26 verts sur les deux fichiers de test.

### Infra
- **`tests/vanilla/bottom-sheet.test.js` + `tests/vanilla/tabs.test.js`** créés (n'existaient pas encore sur `integration/lot-5` au moment de ce ticket — PR #823, vague 4 de #744, encore ouverte). Repartis du contenu de cette PR pour rester alignés, puis complétés avec les 11 tests ci-dessus. `tests/vanilla/helpers/load-components.js` : ajout de `fireTouch` (jsdom n'implémente pas `Touch`, nécessaire pour simuler le swipe du handle). `tests/vanilla/bottom-sheet.test.js` introduit localement un helper `clickTrigger` (focus explicite avant clic), repris à l'identique du pattern déjà documenté dans `modals.test.js` : jsdom ne simule pas le focus-on-click implicite d'un vrai navigateur, or `openSheet()` capture `document.activeElement` en tout début d'appel.
- `shared/components-registry.json` régénéré (`node bin/generate-registry.js`) : synchronise le pont `module[]` avec les classes `.bg-cat-*`/`.border-cat-*` déjà en CSS depuis #800 mais jamais indexées sur cette base (drift préexistant, sans lien avec #825/#826).
...
- **`--chart-1` à `--chart-5`** (`shared/css/tokens.css` + `themes/{acssi,nhood}.json`) : les 5 tokens sont désormais déclarés explicitement dans les 4 couches de cascade pour les 3 thèmes (`:root`/`[data-mode="light"]` pour MSYX qui vit hors `[data-theme]`, `[data-theme="X"]`/`[data-theme="X"][data-mode="light"]` pour ACSSI/NHOOD) et passent ≥3:1 vs `--surface-solid` sur les **6 combos**. Avant/après par combo (min contraste série vs `--surface-solid`) :
  ...
  | acssi-dark | 1,00:1 (`--chart-5` = `--surface-solid`, série invisible) | 3,33:1 | `--chart-5` `#00457a`→`#5b9bd5` — **teinte conservée** (dérive 2,3° vs l'original), voir note ci-dessous ; `--chart-1/2/3/4` inchangés |
  | acssi-light | 3,30:1 (déjà conforme) | 3,30:1 | inchangé |
  | nhood-dark | 2,87:1 (`--chart-5` sous le seuil) | 3,57:1 | `--chart-5` `#7c3aed`→`#8b5cf6` (éclairci, même teinte à 0,3° près, contraste propre 2,87:1→3,86:1) ; le minimum du combo après correction est en réalité `--chart-1` (3,57:1, inchangé) — `--chart-1/2/3/4` inchangés |
  ...
  **Note teinte conservée (ACSSI dark `--chart-5`)** : une première version de cette PR déplaçait `--chart-5` vers un violet (`#a78bfa`), en justifiant qu'`#00457a` n'était qu'à 4° de teinte de `--chart-2` (`#38bdf8`). Cette prémisse était fausse — vérifiée deux fois avec les fonctions OKLCh de `bin/check-categorical-palette.js` lui-même, l'écart réel est de **17,1°** (ΔH OKLCh), très au-dessus du seuil C1 (≥6°) que cette PR instaure pour l'échelle `chart`. Le vrai défaut de `#00457a` n'a jamais été sa teinte : c'était son contraste de **1,00:1** avec `--surface-solid`, les deux tokens partageant littéralement la même valeur (`#00457a` était aussi `--surface-solid`/`--primary-light` du thème). Correction retenue : `--chart-5` reste dans la même famille de bleu marine (`#5b9bd5`, dérive de teinte 2,3° vs l'original), avec luminosité et chroma relevées pour atteindre 3,33:1 — ΔH ≥ 14,8° et ΔE ≥ 0,094 vs les 4 autres séries ACSSI dark (marge confortable au-dessus des seuils C1/C2 ≥6°/≥0,08), et ΔE = 0,106 vs `--cat-5` (`#8b86ff`, échelle catégorielle qui coexiste sur `pages/fondation.html`) pour rester distinct des deux échelles.
...
### Added
- **`tests/vanilla/`** (#744) : première couverture de test du JS vanilla (`shared/components.js`, ~6700 lignes, ~40 composants, zéro test jusqu'ici). Outillage : Vitest réutilisé (déjà présent pour `packages/react`, hoisté par les workspaces npm) — aucun second outil introduit ; `vitest`/`jsdom` en devDependencies root, le JS et le CSS distribués restent sans dépendance. Point technique : `shared/components.js` n'est pas un module ES (fonctions globales exposées via `window.__initX`) — chargement via une fenêtre jsdom fraîche par test + eval indirect du fichier réel, équivalent d'un `<script src>`. Couverture initiale : `initContextMenu` (7 tests), `initCalendar` single+range (9 tests), `initJsonViewer` (7 tests) = 23 tests. Nouveau job CI `vanilla-tests` **bloquant**. Preuve par mutation (exécutée puis restaurée) : `show`→`showX` casse 5 tests, retrait de `.selected` sur `.range-start` casse 2 tests, inversion du toggle `open` casse 2 tests — les tests détectent bien les classes d'état dont dépend le CSS, la classe de bug qui a coûté le plus cher cette semaine. Le ticket #744 **reste ouvert** : découpage par famille de composants proposé pour la suite du chantier.

### Fixed
- **Convention `.btn` retirée** (#777) : `.btn` était documentée aux consumers (`docs/DS-PRINCIPLES.md`, `shared/CONSUMER_GUIDE.md`) comme classe root des boutons, mais n'a aucune règle CSS — les 15 variantes `.btn-*` sont autonomes. La convention était de surcroît déjà incohérente dans le HTML du DS lui-même (`btn btn-danger`, `btn-danger btn-sm`, `btn-primary btn-danger`). Arbitrage Mike : aligner la doc sur le code plutôt qu'écrire la règle manquante (l'option inverse aurait imposé de restructurer 15 variantes et de trancher les combinaisons de forme — chantier de refonte, hors scope). Doc corrigée, token `.btn` retiré des usages HTML/JS (`pages/formulaires.html`, `pages/overlays.html`, `shared/components.js`, `shared/components-registry.json`), structure réelle documentée (forme / couleur / taille). Vérification décisive : job visual vert, **0 baseline VR divergente** — confirme empiriquement qu'aucune règle ne ciblait `.btn`.
- **`shared/components-registry.json` : 21 champs `example` fautifs** (#781) : → 0. Traités en 3 groupes. Groupe A (2) — `cssClasses` obsolètes après le renommage #519 : `zone-banner` et `upgrade-prompt` pointaient vers `.zone-*`/`.upgrade-banner*` au lieu de `.alert--kpi`/`.alert--cta`. Groupe B (10) — motif `data-{composant}-id` : attributs cités dans les exemples mais jamais lus par le JS (qui scanne par classe), retirés. Groupe C (9) — vérifiés un par un : 5 réellement morts (retirés), 4 lus sous un autre nom donc corrigés vers le vrai (`data-copy-target`→`data-copy`, `data-rating`→`data-value`, `data-counter-target`→`data-target`, `video-embed`→`data-src`). L'`example` de `copy-button` était non seulement faux mais cassant : sans `.copy-icon`/`.copy-tooltip`, `doCopy()` lève une exception au clic. Divergence relevée en cours de traitement : l'arbitrage initial annonçait `.context-menu-trigger` comme déclencheur réel — faux, `initContextMenu()` utilise `.context-target` ; `.context-menu-trigger` n'existe que dans un commentaire obsolète.

## 2.118.0 — 2026-07-27 — Fin de backlog assainissement : classes mortes, registre BEM, scission calendrier (#775, #776, #747, #748, #770)

> Sprint 6 « Fin de backlog ». Cinq correctifs courts qui referment la dette identifiée par les garde-fous des sprints précédents : deux classes mortes détectées par `check-dead-classes.js` (#765) traitées (#775, #776), un scanner anti-fantôme du registre aveugle par construction aux classes BEM (#747) + une nouvelle validation warn-only du champ `example` (#748), et une scission d'entrée registre (#770) qui avait fait sous-estimer une précédente épopée de portage React.

### Fixed
- **`shared/css/components/lists.css`** (#775) : `.initially-hidden` (posée par `initActivityFeed`, `shared/components.js`, sur les items non chargés du fil d'activité) n'avait aucune règle CSS — le bouton « Charger plus » de la démo ne masquait donc rien, tous les items étaient visibles dès le chargement de la page. Ajout d'une règle dédiée groupant `.activity-item.hidden` et `.activity-item.initially-hidden` : réutiliser `.hidden` seule aurait cassé le filtre par type de la même démo, qui fait un `classList.remove('hidden')` inconditionnel quand `filter === 'all'` — cela aurait révélé prématurément les items non encore chargés. Classes orthogonales, zéro duplication de règle. 31 baselines VR harvestées depuis les actuals CI (la section raccourcit, cascade de décalage sur les sections 16/17/18 de `data.html`).
- **`shared/components.js`** (#776) : retrait de 4 classes posées par le JS sans aucune règle CSS correspondante, détectées par l'inventaire initial de `bin/check-dead-classes.js` (#765) — `.json-tree`, `.json-node--last`, `.json-close-punct` (le port React `<JsonViewer>` #596 les avait déjà auditées et choisi de ne pas les émettre, le vanilla s'aligne désormais) et `.virtual-list-rows` (conteneur de regroupement pur, jamais sélectionné par classe, instruit indépendamment). `role="tree"` vérifié intact sur le nœud qui portait `.json-tree`. Écart React résiduel (`VirtualList.tsx` émet encore `.virtual-list-rows`) ticketé en #783, hors scope de cette version.
- **`bin/generate-registry.js`** (#747) : `extractReactClasses` n'incluait pas `_` dans sa classe de caractères de test ET exigeait la présence d'un tiret — tout token BEM (`Block__element--modifier`) échouait donc silencieusement au test et disparaissait du `Set` de classes détectées, rendant le garde-fou anti-fantôme du registre aveugle par construction sur cette famille de classes. Corrigé, couverture de test dédiée ajoutée. Impact mesuré sur l'inventaire existant : aucune dérive réelle cachée (`<SplitButton>` est le seul émetteur BEM du DS, ses classes existaient déjà correctement dans le registre).
- **`bin/generate-registry.js`** (#748) : nouvelle validation **warn-only** du champ `example` de chaque entrée du registre — les classes citées dans l'exemple de code doivent être cohérentes avec `cssClasses`, et les attributs `data-*` cités doivent être réellement lus par le JS du composant (`shared/components.js`/`shared/nav.js`). Inventaire initial : 21 entrées fautives sur 138 analysées — traitement ticketé en #781, volontairement hors scope de cette version (bascule bloquante prévue via `--example-strict` une fois la dette résorbée, cf. `docs/DS-PRINCIPLES.md` §6.1).
- **`shared/components-registry.json`** (#770) : l'entrée `calendar` du registre lumpait deux composants distincts en une seule entrée, et `initTimePicker` n'y était déclaré nulle part — ce lumping avait fait estimer le ticket de portage React #628 à ~11 SP et avait imposé sa scission ultérieure en #760/#761. Désormais deux entrées distinctes : `calendar` (`.cal-*`, `initCalendar`) et `time-picker` (`.time-input-wrap`/`.time-sep`, `initTimePicker`), classes relevées directement dans le CSS. `REACT_TO_REGISTRY` (`bin/generate-registry.js`) passe en mapping 1:1. Compteur de composants **109 → 110**, `site.html`/`docs/ARCHITECTURE.md` régénérés (`site.html` hors matrice VR, vérifié dans `visual.spec.ts`).

### Notes
- `bin/lib/extract-react-classes.js` : fonctions pures extraites de `bin/generate-registry.js` (#747) pour les rendre testables isolément — le script principal a des effets de bord au chargement.
