...
> Touche `shared/components.js` + `visual-tests/fixtures/card-floating-panel-clip-856.html`. Pas de `packages/react/**` dans cette PR (fix vanilla uniquement — un bug jumeau latent existe dans `ActionMenu.tsx`, signalé en fin d'entrée, non traité ici).
...
- **`card-floating-panel-clip.spec.ts` (#880) échouait en CI : 4 tests en échec déterministe + 8 flaky (run de référence `33195036514`)** — la PR #880 avait été mergée pendant que le job `visual` était annulé (timeout, corrigé depuis par #851), ses propres tests de non-régression n'avaient donc jamais tourné jusqu'au bout. Deux causes distinctes, aucune n'étant un défaut du test :
  ...
  - **Hors périmètre, signalé pour suivi** : `packages/react/src/components/ActionMenu/ActionMenu.tsx:163` porte le même `right: window.innerWidth - rect.right` — bug jumeau latent, pipeline/versioning `packages/react` distincts (non corrigé dans cette PR).
