...
- **`.hidden-mobile` / `.hidden-desktop` (utilities.css — #568)** : utilitaires responsive de masquage, breakpoint 768px aligne sur `.bottom-nav`. Mobile-first : `.hidden-mobile` masque < 768px (revert ≥ 768px) ; `.hidden-desktop` masque ≥ 768px (revert < 768px). Strategie `display:revert` pour eviter les conflits composants (table, flex, etc.).
