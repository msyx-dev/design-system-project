...
shared/graph/           # Moteur graphique node-link — fondations I1a (v2.98.0, #657) + rendu I1b-2 (v2.100.0, #666) + layouts riches I3-1 (v2.101.0, #669) + I3-2 (v2.102.0, #670)
  ...
  render/                 # svg-renderer.js (SvgRenderer : pipeline measure→layout→paint, paint() ASYNC-TOLERANT depuis #670 — extraction _applyLayout() + token anti-course _paintToken, cycle observe→repaint(rAF)→destroy) + node-types.js (resolveNodeType, graphCard) + a11y-table.js (graphToTableModel pur + renderA11yTable DOM) — v2.100.0, #666 + v2.102.0, #670
  index.js                # API publique ESM : createGraph(el, opts) -> {model, destroy, svg} — v2.100.0, #666 (opts.layout union 'fixed'|'tree'|'radial'|'mindmap'|'layered'|'auto', v2.102.0/#670)
  ...
  README.md              # Frontiere de build D1, jalon nexus post-merge (schemaVersion non fige), section Vendoring dagre (#670)
shared/dist/
  graph-lib.global.js    # GENERE (shared/graph/build.sh) mais COMMITE — window.__pointerDrag/__svg, charge avant components.js
  graph.global.js         # GENERE mais COMMITE — window.MSYXGraph (moteur complet), charge UNIQUEMENT sur les pages qui rendent un graphe (#666, +radial/auto #669, +mindmap #670 — dagre/layered restent hors bundle, ~10.2 KB gzip)
...
pages/data.html#graph             # Section demo (fixed + tree + radial #669 + layered/mindmap NHOOD #670), initGraph() dans components.js reinitAll()
docs/adr/
