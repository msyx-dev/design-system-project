...
shared/graph/           # Moteur graphique node-link — fondations I1a (v2.98.0, #657) + rendu I1b-2 (v2.100.0, #666) + layouts riches I3-1 (v2.101.0, #669)
  ...
  layout/                 # fixed.js (lit node.position) + tree.js (Reingold-Tilford naif deterministe) + radial.js (mindmap radiale 360°, racine au centre, anneaux ∝ profondeur, secteurs ∝ charge feuille — v2.101.0, #669) + detect.js (heuristique topologique pure : 1-racine acyclique→tree, DAG/cyclique→layered, vide→fixed ; radial/mindmap jamais auto-choisis) + auto.js (wrapper layout 'auto', degrade vers tree via hasLayout() tant que layered #670 absent) + index.js (registre registerLayout/resolveLayout/hasLayout) — purs, DOM-free, testables Node — v2.100.0/#666 + v2.101.0/#669
  ...
  index.js                # API publique ESM : createGraph(el, opts) -> {model, destroy, svg} — v2.100.0, #666 (opts.layout union 'fixed'|'tree'|'radial'|'auto', #669)
...
shared/dist/
  ...
  graph.global.js         # GENERE mais COMMITE — window.MSYXGraph (moteur complet), charge UNIQUEMENT sur les pages qui rendent un graphe (#666, +radial/auto #669)
...
pages/data.html#graph             # Section demo (fixed + tree + radial #669), initGraph() dans components.js reinitAll()
docs/adr/
...
- **Graph** (`initGraph()`) : filet monolithe — lit `.graph[data-graph] > script.graph-config` (JSON), délègue à `window.MSYXGraph.createGraph(el, cfg)` (`shared/graph/global-entry-engine.js`, no-op si le bundle `graph.global.js` n'est pas chargé sur la page). Le moteur lui-même (`shared/graph/`) : pipeline `measure→layout→paint` (mesure interne, jamais de mutation du modèle `GraphModel` #665), layouts `fixed`/`tree`/`radial` purs DOM-free (`layout/`) + `auto` (auto-détection topologique via `adjacency` : 1 racine acyclique → `tree`, DAG/cyclique → `layered` si enregistré sinon dégradé `tree` via `hasLayout()` — `radial`/`mindmap` toujours explicites, jamais auto-choisis, `fixed` jamais par défaut, #669), rendu SVG + noeud riche `foreignObject`/`graphCard()` + alternative a11y `.graph-table` (`aria-describedby`, contrat primaire), cycle observe(`graph:model:change`)→repaint(rAF debounce)→destroy (`window.__registerInstance`). Couleurs 100% `var(--graph-*)` — repaint gratuit au toggle theme/mode. `dataset.bound`. (#666, #669)
